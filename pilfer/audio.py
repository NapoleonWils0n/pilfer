...
def ffmpegaudio(**kwargs):
    ...
    ext = 'mka'
    recordingfile = os.path.join(home, desktop, 'audio-{}.{}'.format(time, ext))
    ...
    if sys.platform.startswith('linux'):
        ...
        time = datetime.now().strftime("%Y-%m-%d-%H-%M-%S")
        ext = '.mka'
        recordingfile = home + desktop + 'audio-' + time + ext
    ...
    ffcmd = "{0} -hide_banner -stats -v panic -i {1} -vn -c:a copy {2}".format(ffmpeg, url, recordingfile)
    ...
    if any(word in kwargs for word in ('user-agent', 'referer', 'cookie')):                 
        ...
        ffcmd = "{0} -hide_banner -stats -v panic {2} -i {1} -vn -c:a copy {3}".format(ffmpeg, url, options_join, recordingfile)
    ...
    if 'duration' in kwargs:
        ...
        remove_bracket = str(options)[1:-1]
        options_join = ''.join(remove_bracket).replace('"', '')
        ffcmd = "{0} -hide_banner -stats -v panic {2} -i {1} -vn -c:a copy {3} {4} {5}".format(ffmpeg, url, options_join, tflag, duration, recordingfile)
...
def rtmpaudio(**kwargs):
    ...
    if 'duration' in kwargs:
        tflag = kwargs['tflag']
        duration = kwargs['duration']
        rtmpcmd = "{0} -q -i '{1}' | {2} -hide_banner -stats -v panic -i - -vn -c:a copy {3} {4} {5}".format(rtmpdump, url, ffmpeg, tflag, duration, recordingfile)
